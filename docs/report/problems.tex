...
Work related to dynamic programming can be separated in two categories: ad-hoc implementations and grammar-based implementations. Grammar-based dynamic programming was initiated sparked by \cite{adp_discipline} and implemented as a Haskell DSL \cite{adp}. To overcome performance issues, multiple solutions were devised: converting the grammar to C or CUDA code \cite{adp_gpu} or using loop fusion to improve Haskell performance \cite{adp_fusion}. Ultimately, the dynamic programming algebra and grammar were formalized into an external DSL 
and ad-hoc compiler
...
Related work is a DSL embedded in Haskell \cite{adp} with possible conversion to CUDA code \cite{adp_gpu}, a compiler for a dynamic programming external DSL into C code \cite{gapc} or ad-hoc CUDA implementations for specific problem classes \cite{swat_mega}, \cite{gpu_atlp}.
