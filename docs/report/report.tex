...
\subsection{Metrics}
As the device memory is quite limited, it seems interesting to take into account the space usage. The space requirement limits the maximal size of addressable problems  on a particular hardware. This might be a concern for large problems, because they would require special adaptation to handle such cases both correctly and efficiently. However, simple solutions like using a device with larger memory or using main memory (if $5\times$ slowdown is still acceptable) could solve this issue.
...
Test 1 : Ad-hoc VS CUDA parsers (implies ATLP rescaled comparison)

Test 2 : CUDAlign vs CUDA parsers for small sequences

Test 3 : GAPC vs Java parsers vs CUDA parsers

Test 4 ? : GPU RNAfolding vs CUDA parsers
