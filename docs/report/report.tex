...
\begin{verbatim}
NEW:
- CPU: Dynaprog vs ADPFusion vs RNAfold vs GAPC
- GPU: (Zuker) Dynaprog vs GAPC (si on arrive a faire marcher)
- GPU: (MatMult) Dynaprog vs ATLP
- GPU: (SWat) Dynaprog vs  CUDAlign
\begin{verbatim}
...
\subsubsection{Test 1 : Ad-hoc VS CUDA parsers (implies ATLP rescaled comparison)}

\subsubsection{Test 2 : CUDAlign vs CUDA parsers for small sequences}
Also CUDASW++

\subsubsection{Test 3 : GAPC vs Java parsers vs CUDA parsers}

\subsubsection{Test 4 ? : GPU RNAfolding vs CUDA parsers}
