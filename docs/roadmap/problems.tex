...
\item Vectors of size $n$ are indexed from 0 to $n-1$, matrices follow the same convention
...
\item Optimizations: we might want to normalize the semantics of indices and use $(n+1) \times (n+1)$ matrices where the meaning of cell $(i,j)$ would be $\underset{i\le k<j}{\rm chain}(A_k)$.
	%\ul
...
% ----------------------------------------------
\newpage
\subsubsection{Nussinov algorithm}
