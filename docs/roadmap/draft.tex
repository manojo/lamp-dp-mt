...
\subsection*{Abstract}
Dynamic programming (solving a problem by leveraging subproblems solutions) is a common pattern used in various domains. Yet underlying matrix recurrences are non-trivial and error prone. Additionally, domain experts might not have the skills to make an efficient parallel implementation. In this paper, we leverage the Scala / LMS infrastructure to implement a domain specific language (DSL) for dynamic programming on heterogeneous platforms, which allows to write concise and efficient programs. Our contributions are: \ul
\item Classification of dynamic programming problems, and generalization of their characteristics % shape, dependency graph
\item State of the art parallel implementation of these classes on GPUs
\item Grammar embedded in Scala (DSL) to express dynamic programming problems efficiently
