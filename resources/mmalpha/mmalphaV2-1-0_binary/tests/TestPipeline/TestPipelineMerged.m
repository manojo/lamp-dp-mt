...
res = 
{
  ...
  p1 = pipeall["C", "b.(i,j->j)", "BB.(i,j->i+1,j)"];
  If[ p1===<<MV61.ast,
    Print["Test 1 OK"];True,Print["Problem with Test 1"];False]
,
  load["MV1.alpha"];
  p2 = pipeall["C", "b.(i,j->j)", "BB.(i,j->i+1,j)", "{i,j| i>=-1}"];
  If[ p2===<<MV62.ast,
    Print["Test 2 OK"];True,
    Print["Problem with Test 2"];False]
,
  load["MV1.alpha"];
  p3 = pipeall[{"C"}, "b.(i,j->j)", "BB.(i,j->i+1,j)"];
  If[ p1===p3,
    Print["Test 3 OK"];True,
    Print["Problem with Test 3"];False
  ]
,
  load["MV1.alpha"];
  p4 = pipeall[{"C"}, "b.(i,j->j)", "BB.(i,j->i+1,j)", "{i,j| i>=-1}"];
  If[ p4===p2,
    Print["Test 4 OK"];True,Print["Problem Test 4"];False
  ]
,
  load["MV1.alpha"];
  p5 = pipeall[$result,"C", "b.(i,j->j)", "BB.(i,j->i+1,j)"];
...
,
  ...
  Print["Testing pipeline function"];
  p1=First[Position[$result,readExp["b.(i,j->j)",{"N"}]]];
  Print["pipeline b in C with pipe vector (0,1), should fail "];
  pipeline[p1,"B1.(i,j->i,j+1)"];
  If[ prog1=!=$result,
    Print["Problem with test 11"];False,
    Print["test 11 OK \n \n"];True
  ]
,
Print["pipeline b in C with pipe vector (1,0), should succeed"];
nprog1=pipeline[p1,"B1.(i,j->i+1,j)"];
If [nprog1=!=<<8.ast,
    Error=2;Print["Problem with test 12"];False,
    Print["test 12 OK\n \n"];True],
Print["Testing pipeall function"];
prog2=<<2.ast;
prog1=First[load["MV1.alpha"]];
...
pipeall["C","b.(i,j->j)","B1.(i,j->i,j+1)"];
If [prog1=!=$result,
    Error=3;Print["Problem with test 13"];False,
    Print["test13 OK\n\n"];True];
load["MV1.alpha"];
Print["pipeline b in C with pipe vector (1,0), should succeed"];
pipeall["C","b.(i,j->j)","B1.(i,j->i+1,j)"];
If [prog2=!=$result,
    Error=4;Print["Problem with test 14"];False,
    Print["test 14 OK\n\n"];True];
...
prog1=<<4.ast;
If [prog1=!=sys1,
    Error=1;Print["Problem with test 15"];False,
    Print["test 15 OK \n \n"];True],
sys1=pipeIO["c",readExp["C"],
       readExp["C2.(i,j,N->i+1,j+1,N)"],readDom["{i,j,N | i <=N}"]];
If [prog1=!=$result,
    Error=2;Print["Problem with test 16"];False,
    Print["test 16 OK \n \n"];True],
load["MV1P2.alpha"];
sys1=pipeIO[$result,"c","C","C2.(i,j->i+1,j+1)","{i,j | i <=N}"];
If [(prog1=!=sys1)||($result=!=prog0),
    Error=3;Print["Problem with test 17"];False,
    Print["test 17 OK \n \n"];True],
load["MV1P2.alpha"];
sys1=pipeIO["c","C","C2.(i,j->i+1,j+1)","{i,j | i <=N}"];
If [prog1=!=$result,
    Error=4;Print["Problem with test 18"];False,
    Print["test 18 OK \n \n"];True],
load["MV1P1.alpha"];
(* $result >> 6.ast *)
prog0= << 6.ast;
...
prog1=<<7.ast;
If [prog1=!=sys1,
    Error=1;Print["Problem with test 19"];False,
    Print["test 19 OK \n \n"];True],
sys1=pipeIO["B1",readExp["b.(i,j,N->j,N)"],
       readExp["B2.(i,j,N->i+1,j+1,N)"],readDom["{i,j,N | j >=0}"]];
If [prog1=!=$result,
    Error=2;Print["Problem with test 20"];False,
    Print["test 20 OK \n \n"];True],
load["MV1P1.alpha"];
sys1=pipeIO[$result,"B1","b.(i,j->j)","B2.(i,j->i+1,j+1)","{i,j | j >=0}"];
If [(prog1=!=sys1)||($result=!=prog0),
    Error=3;Print["Problem with test 21"];False,
    Print["test 21 OK \n \n"];True],
load["MV1P1.alpha"];
sys1=pipeIO["B1","b.(i,j->j)","B2.(i,j->i+1,j+1)","{i,j | j >=0}"];
If [prog1=!=$result,
    Error=4;Print["Problem with test 22"];False,
    Print["test 22 OK \n \n"];True],
