...
(*      $Author: quinton $
	$Date: 2009/05/22 10:23:51 $
	$Source: /local/chroot/cvsroot/irisa/mmalpha/lib/Packages/Alpha.m,v $
	$Revision: 1.12 $
	$Log: Alpha.m,v $
	Revision 1.12  2009/05/22 10:23:51  quinton
	May 2009
	
	Revision 1.11  2008/12/29 17:25:02  quinton
	The tests were deeply modified
...
Unprotect[add, affine, and, binop, boolean, bottom, call, case, const,
  ...
  dependuse, fixedPointOp, $testResult
];
...
$showGraph::usage = 
"$showGraph is a global variable that is set by the report function. Use is not
garanteed.";

$testResult::usage =
"$testResult is a global variable that is used to store the accumulated 
...
$testIdentifier::usage = 
"$testIdentifier contains the name of the test that one wants to perform.
Default value is \"\".";

$testReportFile::usage = 
"$testReportFile is the name of the reporting file for the tests. Its 
value is \"\" when no report file is open, otherwise, it contains the";
...
$vhdlPatternsDir = 
"$vhdlPatternsDir is the name of the directory where the VHDL pattern 
files is located. This variable is set in Alpha.m"; 
...
mute::usage = "mute is an option (Boolean). If True, function prints absolutely no information."

test1::usage = "test1[] executes part 1 of tests. See documentation in doc/Tests.";
test2::usage = "test2[] executes part 2 of tests. See documentation in doc/Tests.";
test3::usage = "test3[] executes part 3 of tests. See documentation in doc/Tests.";
test4::usage = "test4[] executes part 4 of tests. See documentation in doc/Texts.";
...
(*
  Uncomment this if you want to be informed of the creation of any
  new symbol in the Global context. Useful to track parasital declarations.
$NewSymbol := 
  If[#2 === "Global`", Print["Name: ", #1, " Context: ", #2]] &;
 *)
...
demoLink[s:_String]:=
  demoLink[s, s];
demoLink[d:_String, s:_String]/;($VersionNumber < 6.):=
Catch[ 
Module[{fullname},
...
Module[{fullname},
  ...
  Button[s, NotebookOpen[ $demoDirectory<>fullname ] ]
  ]
];
...
docLink[s:_String]:=
  docLink[s, s];
docLink[d:_String, s:_String]/;($VersionNumber < 6.):=
Catch[ 
Module[{fullname},
...
Module[{fullname},
	...
	If[ FileInformation[ fullname ] === {}, 
	    Throw[ Message[ docLink::file ] ]];
        Button[s, NotebookOpen[ fullname ] ]
  ]
];
...
link[d:_String, s:_String]/;($VersionNumber < 6.):=
Catch[ 
Module[{fullname},
...
(* Definition of test *)
Options[ tests ] = {debug->False,verbose->True,mute->False};
...
Module[
  ...
  Print["This test checks that domlib is properly installed"];
  Print["You will find in directory: ", Directory[], " a file ",
    $testReportFile, " where more details are given."];  
 
  SetDirectory[$testDirectory];
  res = tests["Install"];
  If[ res === False, Print["Error " ]; Return[ False ] ];

  res = tests["Domlib"];

  SetDirectory[currdir];

  If[ res, Print["Test was successful"],
    Print["Test was wrong"];
    Print["You will find in directory: ", Directory[], " a file ",
    $testReportFile, " where more details are given."]
  ];
...
test1[___] := Message[ test1::params ];
...
Module[
  ...
  Print["This test checks various basic MMAlpha packages "];
  ...
  res = Apply[And,Map[tests[#1,opts]&,f1]];
...
test2[___] := Message[ test1::params ];
...
test3[___] := Message[ test1::params ];
...
test4[___] := Message[ test4::params ];

(* When no test identifier is supplied, assume it is "" *)
tests[package:_String,opts:___Rule]:=
  tests[ package, "", opts ];

tests[package:_String,identifier:_String, opts:___Rule]:=
Module[ {res, currdir, muteOpt, dbg, vrb, str},

  muteOpt = mute/.{opts}/.Options[ tests ];
  dbg = debug/.{opts}/.Options[ tests ];
  vrb = verbose/.{opts}/.Options[ tests ];

  (* Save directory *)
  currdir = Directory[];

  (* Set the report file as a global variable. This is a trick 
     to avoid specifying in the test files this report file *)
  (* Try to correct the case when the report file has not been 
    properly closed *)
  If[ $testReportFile === $Failed, 
    str = Streams[];
    If[ Length[str]>=1, str = Last[str] ];
    If[ MatchQ[str, OutputStream[__] ], str = First[ str ] ];
    If[ StringMatchQ[ str, Environment["MMALPHA"]<>"*"<>"TestingReport.txt" ],
      Close[ Streams[][[-1]] ]; Print["Test report file was Closed"] ];
  ];

  (* Now, everything should be OK... *)
  Check[ 
    $testReportFile = OpenAppend[currdir<>"/TestingReport.txt"],
    tests::testrepfileopen = "could not open $testReportFile. It may be still open. Look at Streams[], and try Close[Streams[][[-1]]], or restart MMALPHA";
    Message[ tests::testrepfileopen ];
    SetDirectory[ currdir ];
    Close[ $testReportFile ];
    Return[ False ];
  ];

  (* Set the test identifier. If all tests are executed, this is 
   "", otherwise, it should be the nam of the test *)
  $testIdentifier = identifier;

  If[ vrb&&!muteOpt, Print[ "Test report file is in ", 
    currdir<>"/TestingReport.txt"] ];

  WriteString[ $testReportFile, "------> Testing: ", package ];
  ...
  Catch[
    Module[{p,tp},
      ...
      Check[
        SetDirectory[$testDirectory<>"Test"<>package],
        tests::nodir = "Test does not exist";
        Message[tests::nodir]; 
        Close[ $testReportFile];
      ...
      If[ vrb&&!muteOpt, Print["Calling Test: ",p] ];
      WriteString[ $testReportFile, "\nCalling Test", p ];

      $testResult = True;

      Check[
        Get["Test"<>p]; res = $testResult;
      ,
        res = $testResult = $Failed
      ,
        {testFunction::abntest, tests::testnf}
      ];
    ...
    If[ !muteOpt,
      If[res,
        ...
        Print["*********** > There was an error during the test of package: ",
        package]
      ]
    ];
    ...
    If[res,
        WriteString[ $testReportFile, 
          "\n------------- > Test for ",package," was passed successfully."],
        WriteString[ $testReportFile, 
          "\n*********** > There was an error during the test of package: ",
        package]
    ];
...
(* This prevents testFunction to evaluate its arguments in 
  a systematic way, when called *)
SetAttributes[ testFunction, HoldAll ];

testFunction[ exp : _, result : _, testId : _String, opts:___Rule] := 
Module[{vrb, e, r},

  (* If called when $testIdentifier is not the empty string, 
    then test is only evaluated if the identifier is equal the 
    testId argument 
  *)
  If[ $testIdentifier =!= "" && $testIdentifier =!= testId, 
    Return[ "*****" ] ];

  If[ $testIdentifier =!= "", 
    Print[ "Evaluating test : ", 
      "testFunction[ ", HoldForm[ exp ], ", ", HoldForm[ result ], ", ",
      FullForm[ testId ], 
      If[ {opts} === {} , "", ToString[ opts ] ],
      " ]" ] ];
  ...
  If[e =!= r, 
    Message[testFunction::abntest,testId];
     If[vrb, Print["Returned value was: \n", FullForm[e],
		   "\nExpected value was:\n", FullForm[r]]];
    If[ $testReportFile =!= Null, 
      WriteString[ $testReportFile , "\n****** > test ",testId, " was wrong"]
    ];
    False, 
    If[ vrb, Print["--------> Passed test ",testId] ];
(*
    If[ $testReportFile =!= Null, 
      WriteString[ $testReportFile, "\n--------> Passed test ",testId]
    ];
...
load[filename:_String/;StringMatchQ[filename,"*.vhdx"], opts:___Rule] :=
Catch[
  Module[ {st},
    Check[
      st = ReadList[ filename, Record, RecordSeparators -> {"/\n"} ],
      Message[ load::wrongFile, "File `1` does not exist" ];
      Throw[ Message[ load::wrongFile, filename ] ]      
    ];
