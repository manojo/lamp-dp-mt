...
(* Standard head for CVS
	...
	$Author: quinton $
	$Date: 2010/04/14 18:01:48 $
	$Source: /local/chroot/cvsroot/irisa/mmalpha/lib/Packages/Alpha/Semantics.m,v $
	$Revision: 1.5 $
	$Log: Semantics.m,v $
	Revision 1.5  2010/04/14 18:01:48  quinton
	Minor modifications.
	
	Revision 1.4  2007/06/12 08:47:04  trisset
	tanguy modifs
	
	Revision 1.3  2006/05/29 21:47:59  quinton
	There was an error in function matchType (see inside package)
	
	Revision 1.2  2005/04/29 10:26:19  trisset
	added setBitWidth
	
	Revision 1.1.1.1  2005/03/07 15:32:13  trisset
	testing mmalpha repository
	
	Revision 1.66  2004/09/16 13:44:58  quinton
	Updated documentation
	
	Revision 1.65  2004/08/02 13:14:03  quinton
	Documentation updated for reference manual
	
	Revision 1.64  2004/06/16 14:57:32  risset
	switched to ZDomlib
...
Semantics::usage = 
"Alpha`Semantics` is the package which contains the functions for
computing the type of Alpha expressions. These functions are changeType,
expDimension, expDomain, expType, getContextDomain, matchTypes, 
and replaceByEquivExpr.";

changeType::usage = 
"changeType[sys,var,newType] changes the type of the variable var
to newType in sys (default $result). No typing compatibility check 
is done: this is not a semantic preserving transformation. Mainly 
used for changing integers to fixed bit width integers.";

expDimension::usage = 
"expDimension[sys,exp] computes the dimension of the expression exp the in 
system contained in the system sys (default $result). The expression
...
expDimension outputs error messages if the dimensions are not compatible,
...
sys (default $result) or $Failed if the expression is badly formed.
expDomain generates error and warning messages accordingly. The
...
real, bottom) of an expression and checks its correctness
using type--matching rules. Default value of sys is $result.
The expression may be given as a string, a position 
vector (following the convention of the Mathematica function Position)
in the system, or as an AST. expType returns bottom if the
...
"getContextDomain[sys,pos] computes the context domain in which an
expression at position pos in sys is used. Default value of sys is
$result. The context is the domain inherited by an expression from
the constructs which surrounds it.  The position is counted from the
root of the system following the Mathematica notion of position.";

matchTypes::usage =
"matchTypes[type1,type2] computes the type corresponding to the union
of those of the parameters (integer, boolean, real) and returns the 
resulting type or bottom if the types are incompatible.";

expDomain::ERROR = " ";

replaceByEquivExpr::usage= 
"replaceByEquivExpr[sys,pos,expr] replace in sys the expression
present at position pos by expr. Default value of sys is $result.
replaceByEquivExpr[sys,expr1,expr2] replace in sys all occurences of
expr1 by expr2. expr1 and expr2 can be given in AST form of Alpha form. 
Warning: the equivalence of the resulting program is NOT
guaranteed. A test is made to check if both expressions are
equivalent, but if this test fails, the replacement is done anyway.
This function should be used for replacing an expression by an equivalent
one when this cannot be done automatically (for instance, in presence
of degenerated domains).";

setBitWidth::usage="setBitWidth[var:_String,value:_Integer] change the
scalar type of variable var1 in system sys if this variable has a
specified bitwidth (e.g. integer[S,5])
and set it to the bitwidt value, if the variable
does not have a specified bitwidth, it does nothing and issue a warning"

Begin["`Private`"]

Clear[setBitWidth]

Options[setBitWidth]={debug->False}

setBitWidth[var1:_String, value:_Integer,options___Rule]:=
Module[{res},
       res=setBitWidth[$result,var1,value];
       If [MatchQ[res,system[___]],
           $result=res,
           res=$result];
        res ]

setBitWidth::typeNotSuitable="Sorry, type `2` of var `1` cannot be set to a given bitwidth"

setBitWidth[sys:_system,var1:_String, value:_Integer,options___Rule]:=
Module[{res,decl1,oldType,newType,newDecl,pos1},
...
If[!MatchQ[decl1,decl[___]],
   ...
   Switch[oldType,
      ...
      (_[_String, _Integer]),
      (* integer[S,x]->integer[S,value] *)
      newType=ReplacePart[oldType,value,2];
      newDecl=ReplacePart[decl1,newType,2];
      pos1=Position[sys,decl1];
      If[Length[pos1]=!=1,
            Message[setBitWidth::error];
            Print[pos1];
            res=sys,
            res=ReplacePart[sys,newDecl,First[pos1]]],

      (_[_String, _Integer,_Integer]),
      Message[setBitWidth::typeNotSuitable,var,oldType];
      res=sys,
...
setBitWidth::wrongArg="wrong Argument for function setBitWidth : `1` "

setBitWidth[a:___]:=Message[setBitWidth::wrongArg,Map[Head,{a}]]
...
(*---------------------------------------------------------------------*)
(* There was a bug here, after the modifications regarding the bitwidth.
   Indeed, the matchTypes function was modified so that the operator 
   was introduced as a third argument, to compute the bitwidth of S or
   U integers. But when the type was simple, provision was not made to 
   ensure that the coercion rules still apply. I added extra definitions
   to cover these cases. 
*)
Clear[matchTypes];
matchTypes[integer, integer] := integer;
matchTypes[integer, integer, _] := integer; (* Correction PQ May 2006 *)
matchTypes[real,    real]    := real;
matchTypes[real,    real, _]    := real; (* Correction PQ May 2006 *)
(* Type coercion : integer -> real *)
matchTypes[integer, real] := real; 
matchTypes[integer, real, _] := real; (* Correction PQ May 2006 *)
matchTypes[real, integer] := real;
matchTypes[real, integer, _] := real; (* Correction PQ May 2006 *)
matchTypes[boolean, boolean] := boolean;
matchTypes[boolean, boolean, _] := boolean; (* Correction PQ May 2006 *)

matchTypes[boolean, unkown] := boolean;
matchTypes[boolean, unkown, _] := boolean; (* Correction PQ May 2006 *)
matchTypes[integer, unkown] := integer;
matchTypes[integer, unkown, _] := integer; (* Correction PQ May 2006 *)
matchTypes[real, unkown] := real;
matchTypes[real, unkown, _] := real; (* Correction PQ May 2006 *)

matchTypes[a:integer,b:integer[_,_]] := integer; 
matchTypes[a:integer,b:integer[_,_], _] := integer; (* Correction PQ May 2006 *)
matchTypes[a:integer[_,_],b:integer] := integer;
matchTypes[a:integer[_,_],b:integer,  _] := integer; (* Correction PQ May 2006 *)

matchTypes[a:integer[_,_],b:integer[_,_]] := matchTypes[a,b,add];

matchTypes[a:integer[_,_],b:integer[_,_],op:_Symbol] := 
Module[{widthA,widthB,codeA,codeB,widthFinal,codeFinal},
  If [MatchQ[a,integer_] && MatchQ[b,integer_],
    (* Set the bit size of the op *) 
    widthA=a[[2]];
    widthB=b[[2]];
    widthFinal= Switch[op,
                      add,Max[widthA,widthB],
                      mul,widthA+widthB,
             (* all others *)
                      _,Max[widthA,widthB]];
...
Check[
  ...
  getContextDomain::errunion = "error while calling DomUnion";
  Message[ getContextDomain::errunion ]
])
...
(*   dbg = True;  Print["11"]; *)
  ...
  dvar= Part[ getDeclaration[sys, Part[equ,1]], 3]; 

  If[ dbg, Print["Domain of lhs var : ", ashow[dvar,silent->True] ] ];
  dom = 
    Check[ 
      DomExtend[ sys[[2]], 
        If[Head[dvar[[3,1]]]===Alpha`pol, dvar[[2]],dvar[[3,1,1,3]]] 
      ]
    , (* change *)
  ...
  posEq = Part[ Position[ sys, equ, Infinity ], 1 ];
  ...
  res = 
    ...
    ,
      ...
      getContextDomain::errgcd1 = "error while calling getContextDomain1";
      Throw[ Message[ getContextDomain::errgcd1 ]; $Failed ]
    ];
...
getContextDomain1[
  ...
  occur:{pos_, poss___},
  opts:___Rule ]:=
Module[ {branch1, rest, dbg, listOfDomains, lg, overlap, chck, eq}, 
  dbg = debug/.{opts}/.Options[getContextDomain];
...
(*  dbg = True; Print["1111"]; *)
  ...
  (* This is a kludge 
  eq = Cases[ sys, equation[ x:_, e ]->x, 2 ];
  If[ dbg&& eq =!= {}, Print[ "This case expression is in definition of: ", eq[[1]] ] ];
  eq = eq[[1]];
  *)
  ...
  If[ chck, 
    ...
    *)
    ...
    If[ lg > 1, 
      listOfDomains = 
        Map[ 
	    Function[ { exx }, DomIntersection[ dom, 
             expDomain[ sys, exx ] ] ]
          , {exp} 
        ];
...
Catch[
  Module[ {tmp, paramdom, paramlist, test},
    If[ !expEmpflg,
      If[ DomEmptyQ[dom],
      ...
      , (* Else look if the domain is empty for some values
        ...
        paramlist = If[Head[paramdom[[3,1]]]===Alpha`pol,
          paramdom[[2]],paramdom[[3,1,1,3]]];
        tmp = 
          DomDifference[
            paramdom,
...
(* 
  Checking done on Feb. 25, 2006 by P. Quinton. This function
  does not work when the expression contains a z polyhedron
*)
Clear[expDomain];
