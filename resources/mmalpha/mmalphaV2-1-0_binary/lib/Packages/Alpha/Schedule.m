...
(* Standard head for CVS
   ...
   $Author: quinton $
   $Date: 2010/04/10 20:06:11 $
   $Source: /local/chroot/cvsroot/irisa/mmalpha/lib/Packages/Alpha/Schedule.m,v $
   $Revision: 1.4 $
   $Log: Schedule.m,v $
   Revision 1.4  2010/04/10 20:06:11  quinton
   Minor editions.

   Revision 1.3  2008/04/25 16:39:20  quinton
   Added a mute option, when True, absolutely no messages.

   Revision 1.2  2007/04/09 18:47:36  quinton
   Modifications to have schedule representations which are uniform in Farkas and Vertex Schedulers
...
structSched::usage = "structSched[] tries to find a structured
...
schedule::usage = 
"schedule[] finds the schedule of $result and puts the result in 
$schedule. schedule[sys_Alpha`system], finds the schedule of alpha system 
`sys' and puts the result in $schedule (affine by variable by default).  
schedule[sys_Alpha`system, options] calls schedule with non default 
options. Options[schedule] provides the options of schedule, and 
?opt1 provides information on option opt1. ?$schedule provides info 
on the output format of the schedule. The schedule computation may 
take a long time (2 minutes for 20 instructions). More information 
is available in the file $MMALPHA/doc/user/²Scheduler_user_manual.ps."
...
(*
   This was replaced by a call to storeSchedule
 ...
 *)
      ...
      (* Add new schedule to the library. Modification done by 
         P.Q. on February 2006 *)
      If[ MatchQ[$schedule,_scheduleResult],
        $scheduleLibrary = 
          storeSchedule[ sys0, sys0[[2]][[2]], $schedule, options ],
        schedule::wrgresult = "wrong schedule. Report bug to Alpha";
        Throw[ Message[ schedule::wrgresult ] ]
      ];
...
(*
  This function was cut an pasted from VertexSchedule.
  It allows one to add a schedule to $scheduleLibrary, in 
  the same form as vertex method.
*)
Clear[ storeSchedule ];
storeSchedule::usage = 
...
(*
   Now, we have got the schedule, and we will add it to 
   $scheduleLibrary.
 *)
(*
   This was modified in order to be compatible with the other
   schedules.
*)
(*
      If [MatchQ[$schedule,_scheduleResult],
...
checkOptions[sys_Alpha`system,
	    options___Rule]:=
Module[
  {optDebug,optVerbose,optResolutionSoft,
  ...
  optMute = mute/.{options}/.Options[schedule];
  ...
  (* No Checks actually... just for confidence *)
