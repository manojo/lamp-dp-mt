...
(* Standard head for CVS
	...
	$Author: quinton $
	$Date: 2007/04/09 17:38:52 $
	$Source: /local/chroot/cvsroot/irisa/mmalpha/lib/Packages/Alpha/FarkasSchedule.m,v $
	$Revision: 1.4 $
	$Log: FarkasSchedule.m,v $
	Revision 1.4  2007/04/09 17:38:52  quinton
	The result of a schedule was slightly changed in order to have a standard form, identical for Farkas and Vertex schedulers
...
**        The line are all terminated by " \ ". 
**
******************************************************************************)
...
fichierPip[ fich_String,mat_, refline_,options_List] :=
  Module[{matrice,fichInput1,fichInput2,fichOutput,result,
    matrice2,silentOrNot,dir,command,timePip,dbg,vrb},

    (* Get debug and verbose option *)
    dbg = options[[1]];
    vrb = options[[2]];
    ...
    If[ dbg, Print["Building the pip matrix..."]];
    matrice = matPip[mat,options];

    If[ dbg, Print["Writing input file 1..."]];	 
    Check[ writeFile[ fichInput1, matrice, refline, options], Print[ "File fichInput1 was not written !" ]];
    If[ dbg, Print[ FileInformation[ fichInput1 ] ] ];
	 
    If[vrb||dbg, Print["Solving the LP..."]];

    Which[
      If[ dbg, Print["Unix like system..."]];	 
      $OperatingSystem === "Unix" || $OperatingSystem ==="MacOSX",
      If[ dbg, Print["Remove \ and create fichier input 2"]];	       
      Run["sed -f "<>Environment["MMALPHA"]<>"/sources/Sed/sup_antislash_pip.sed "<>fichInput1<>" > "<>fichInput2]
    ,
      If[ dbg, Print["Windows or MSDOS..."]];	 
      $OperatingSystem == "WindowsNT" || $OperatingSystem == "MSDOS",
      If[ dbg, Print["Remove \ and create fichier input 2"]];	       
      command="sed.exe -f "<>Environment["MMALPHA"]<>"/sources/Sed/sup_antislash_pip.sed "
    ...
    If[ !dbg, DeleteFile[fichInput1] ]; 

    (* If verbose, then run Pip with option -s *)	
    If[ !vrb, silentOrNot=" -s ", silentOrNot="  "]; 
	
    If[ dbg, Print["Run Pip..."]];	 
    timePip = 
      Timing[
        Which[
          $OperatingSystem === "Unix" || $OperatingSystem ==="MacOSX",
...
(* 
	...
	easy-to-read format. The structure of the file is known, the 
	refline (indicating how many phi,tau,alpha,llambda,... are 
	present in the file) is given as second argument and 
	we should retrieve the value of the last variable to all taus
	and alphas.
*)
...
Module[{res,nbPhi,nbInstr,nbTotTau,listNegCoef,
  ...
  dbg = options[[1]];
  If[ dbg, Print["Reading file: ", file ] ];
