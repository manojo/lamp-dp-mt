...
Catch[
  ...
  (* This means that the constraints in the domain do not involve the
     time *)
  res = Apply[ And, Map[ DomUniverseQ, res ] ];
	   *)
...
Catch[
  Module[ { def, eq1, s, h, dbg, sys1 },
    ...
    Print["222222222222222"];
	  ...
	  (* ===========  PQ 18-11-2009 *)
  (* This was added to try obtaining the example of Nussimov...
    sys1 = normalizeDef[ sys, x ];
    ashow[ getDefinition[ sys1, x ] ];
   *)

    def = getDefinition[sys1,x]; (* take the lhs of symbol x *)
    eq1 = getEquation[sys1,x]; (* take the equation defining x *)
...
Catch[
  Module[
    { vec,lin,nn,pos,pp,dom,exp,projvect,dec,dim,tm,
  ...
  nbEqDomExp = 
    Length[ Part[ DomEqualities[ DomConvex[realDomExp] ],4 ] ];
  ...
  *)
  rr = rays[ realDomExp ];
  Print["Rays of domain: ", rr];
 
  ns = NullSpace[ rr ];
  Print["Null space of rays: ", ns];
  ...
  (* If the first one does not work, good idea to try another one... *)
  (* But, this is more difficult. *************** PQ *)
  If[ dbg, 
    Print["List of vectors is: ", vec ];
    Print["I take the First one"]
  ];

  (* Select 1st vector *)
  lin = linHalfSpace[vec[[1]]][[1]];
  If[ dbg, Print["lin half space: "]; Print[ lin ] ];

  (*
    This stuff was removed... It is probably better to use the fit
    procedure of Tanguy see later. *********** PQ 4/10/2009
   (* If ns is not empty, project the linear space on it *)
   If[ ns =!= {},
     Module[ {space, sol},
	     Print["15151515151515"];
	     space = NullSpace[ns];
	     If[ dbg, Print["Complementary space: ", space ] ];
	     sol = LinearSolve[ Join[ space, ns ], lin ];
	     If[ dbg, Print["Decomposition of linear vector: ", sol ] ];
	     lin = lin - Last[sol]*ns[[1]];
	     If[ dbg, Print["Projected vector: ", lin ] ];
     ];
   ];
  *)
  ...
  (* If there is no pipe vector, no pipeline, and we only "delocalize"
    the equation, i.e., we try to remove the processors *)
  If[ Length[nn]===0,
     If[ vrb, 
   ...
   (* By choosing systematically the first vector, one may fail... Thus, we
    try several vectors. *)
  Module[ {i, pipeVectorFound, tempVar, TableVar, domns, nsp},
    i = 1; 
    ...
    While[ (!pipeVectorFound) && (i <= Length[ nn ]), 
      ...
      *) 
      ...
      If[ Length[domEq]>0,
        ...
        (* Here, I force absence of solution, because I do not know if this
          is useful. ************* PQ. 4/10/2009 *)
	  (*        solu1 = {};
	   *)
      ...
      (* If the pipe vector has a non null first component, we 
       keep it. *********** This process may fail ? Otherwise, we return 
       a delocalized control equation. *)
      If[pipe1[[1]]===0,
        Print[pipeControl::nullpipevec,variable];
        delocalizeControl[sys,variable,opts]; 
    ...
    (* If we have exhausted all pipeline vectors, we ere done, and 
  ...
  *)
  ...
  Check[
    ...
    (*
    Print[" 5555555555555555 ",  newVariableName ];
    Throw[On[pipeline::noCheck];
    Message[pipeControl::errpipe];
    sys];

    If[ newVariableName === "X1Xctl2PP", 
      Global`$$system = sys ;
      Global`$$pos = pos ;
      Global`$$name = newVariableName ;
      Global`$$tm = tm;
      Global`$$dom = newDomContext[[1]];
    ];
     *)
...
Clear[pipeAllControl]; 
    Options[pipeAllControl] = {verbose->True, mute -> False, onlyVariables -> {}};
...
Catch[
  Module[{ verb, ctrlist, i, newsys, dbg, muteOpt, lvars },
    ...
    If[ dbg||True, Print["Control variables: ", ctrlist ] ];
    Print[ lvars ];

    If[ lvars =!= {}, ctrlist = Intersection[ ctrlist, lvars ] ];
    If[ dbg||True, Print["Control variables kept: ", ctrlist ] ];
