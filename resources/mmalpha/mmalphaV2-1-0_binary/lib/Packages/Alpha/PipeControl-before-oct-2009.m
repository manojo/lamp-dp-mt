...
Catch[
  Module[ {exp, varname, mat, lp, ns, lg, optdebug, squareQ, inputDep,
    ...
    If[ optdebug || optverbose , Print[ "Equality matrix of domain: ", eqmat ]];
    ...
    (* If dependendy is already uniform, just throw {} *)
    If[
    ...
    If[ 
      (* Input, non square dependency *)
      (inputDep || constantQ) && !squareQ
    ,
      If[ optdebug||optverbose, Print["Calling pipeline IOs "] ];
      (*
      res = tryPipelineIO[ sys, occ, varname, mat, eqmat, opts ]; 
      transfo = "Pipeline IO";
       *)
      res = {};
      If[ res =!= {}, Throw[ res ]]
    ...
    (* First, try a pipeline, just in case. Not sure that this 
      is necessary, otherwise, one or the above possibilities would
      have succeeded... 
    *)
    ...
    *)
    ...
    transfo = "Alignment";
    If[ dbg||vrb, Print["Trying an alignment"]];
    dimlhs = mat[[2]]-1;
    ...
    Module[ { indexes, mat1, params, newindexes, matindexes},
      ...
      If[ ! squareQ, 
        ...
        (* To make sure we try everything -----  *)
        If[ command =!= {},
    ...
    (* 
       Then, we look for a possible uniformization. Try only if
       square
    *)
    If[ squareQ,
