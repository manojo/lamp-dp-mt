...
- onlyVariables option added
- displayInNoteBook option of report added
- usage of isTemporalCaseEquQ was added
- option strictCase of isTemporalCaseEquQ was added
- option strictCase of isSpatialCaseEquQ was added
- 
...
reportDep::usage = 
"reportDep[ repFile, occ, opts ] analyzes a dependency given by 
...
displayInNoteBook::usage =
"displayInNoteBook is an option of report. If False (default value), report is written
only in a file. Otherwise, report is written in the notebook";
...
(* Modification. usage added. Also, options added *)
isTemporalCaseEquQ::usage = 
"isTemporalCaseEquQ[ eq ] is True if equation eq has the form of a temporal
case. If the strictCase option is set to False, the expressions of the 
branches may be any expression, otherwise (default), they have to be of
the form if cond then expr else expt";

Options[ isSpatialCaseEquQ ] = 
  { verbose->False, debug -> False, strictCase -> True };

Options[ isTemporalCaseEquQ ] = 
  { verbose->False, debug -> False, strictCase -> True };

strictCase::usage = 
"strictCase is an option of isSpatialEquQ and isTemporalEquQ. True by
default. If True, the case expression has only a variable as branch
expressions, otherwise, the case expression can be anything.";

(* Modifications. usage completed *)
isSpatialCaseEquQ::usage = 
"isSpatialCaseEquQ[ var ] is True if equation defining var is a spatial case, and can
...
constraints on the processor indexes. There existe a strictCase option to 
accept cases with expression other than variables";

(* ===================== Private definitions ===================== *)

Begin["`Private`"]

(* Modification. Options added *)
Clear[ isSpatialCaseEquQ ];
isSpatialCaseEquQ[ var:_String, opts:___Rule ]:=
Catch[
  Module[ {eq},
    Check[ 
      eq = getEquation[ var ],
      isSpatiatCaseEquQ::notvar = "`1` is not a variable of $result";
      Throw[ Message[ isSpatialCaseEquQ::notvar, var ] ]
    ];
    isSpatialCaseEquQ[ eq, opts ]
  ]
];
isSpatialCaseEquQ[ eq:_equation, opts:___Rule ]:=
Catch[
  Module[ { mtch, doms, res, indx, dbg, vrb, strict },

  (* Getting options *)
  strict = strictCase/.{opts}/.Options[ isSpatialCaseEquQ ];
  dbg = debug/.{opts}/.Options[ isSpatialCaseEquQ ];
  ...
  (* A strict option was added. If not true, expressions other than var are
   allowed in the case expression *)
  mtch = 
    If[ strict, 
      MatchQ[ eq[[2]], 
      ...
      MatchQ[ eq[[2]], 
        case[{restrict[_domain,_], restrict[_domain,_]}]
      ]
    ];   

  If[ dbg, Print[ "Matching case: ", mtch ] ];

  If[ !mtch, Return[ mtch ] ];

  (* Modification *)
  (* We now check that the case relates only on spatial 
   constraints. To do so, we get both domains, we project them
   on processor space, and we check that the intersection of 
   the projected domains is empty : this means that they are
   separated by processor constraints only. *)
  ...
  (* Project the domains over the processor indexes *)
  res = Map[ DomProject[ #, indx ]&, doms ];
  If[ dbg, Print["Domains:\n"]; Map[ show, res ] ];

  (* Check that the intersection of the domains is empty *)
  res = DomEmptyQ[ DomIntersection[ res[[1]], res[[2]] ] ];
...
(* Modification. Added options *)
Clear[ isTemporalCaseEquQ ];
isTemporalCaseEquQ[ var:_String, opts:___Rule ]:=
Catch[
  Module[ {eq},
    Check[ 
      eq = getEquation[ var ],
      isTemporalCaseEquQ::notvar = "`1` is not a variable of $result";
      Throw[ Message[ isTemporalCaseEquQ::notvar, var ] ]
    ];
    isTemporalCaseEquQ[ eq, opts ]
  ]
];
isTemporalCaseEquQ[ eq:_equation, opts:___Rule ]:=
Catch[
  Module[ { mtch, doms, res, indx, dbg, vrb, strict },

  (* Getting options *)
  strict = strictCase/.{opts}/.Options[ isTemporalCaseEquQ ];
  dbg = debug/.{opts}/.Options[ isTemporalCaseEquQ ];
  ...
  (* First, check that the form is a case with 2 branches *)
  (* Modification. Option strict added. If strict, only var expressions are allowed. *)
  mtch = 
    If[ strict, 
      MatchQ[ eq[[2]], 
        case[
  ...
  (* Modification. We check that the domains are separated by 
   time constraints only. To do so, we project the domains on the time
   index, and we check that the intersection is empty *)
  (* Then, get the domains *)
...
isTemporalCaseEquQ[___]:=Message[isTemporalCaseEquQ::params];
...
Module[ {repFile, listOflhs, res, listOfEq, ov },

  (* To clear report file *)
  repFile = OpenWrite[ name<>".report" ];

  ov = onlyVariables/.{opts}/.Options[ report ];
  ...
  listOflhs = Cases[$result, equation[s_, _] -> s, Infinity];
  listOflhs = If[ ov =!= {}, Select[ listOflhs, MemberQ[ ov, # ]& ], listOflhs ];
  ...
  listOfEq = MapThread[ List, {res, Map[ getEquation, listOflhs ]} ];
...
Module[
 {mm, deps, optverbose, optdebug, nonSquareDeps, nonUniformDeps, 
  ...
  optuzabled, optnuzabled, eq, rhs, eqPosition, res, display},
  ...
  optnuzabled = showNonUniformizableDeps /. {opts}
   /. Options[report];
  display = displayInNoteBook/.{opts}/.Options[report];
  ...
  Catch[ 
    ...
    *)
    ...
    If[ deps === {}, 
      ...
      If[ display, Print["  ** No dependency\n" ] ] 
    ];
...
(*
  analyzeEquation is used by the report command. It returns and prints
  information regarding a given equation. 
*)
Clear[ analyzeEquation ];
analyzeEquation[ repFile:_, eq:_, lhs:_, display:_ ]:=
Catch[
  Module[ 
    ...
    If[ localEquation, 
      ...
      If[ display, Print[ "  ** Local equation" ] ]
    ];
    ...
    If[ display, Print[ "  ** Indexes: ", indexes ] ];
    ...
    ,
       ...
       If[ display, Print[ "  ** Scheduled, one time dimension" ] ]
    ,
    ...
    ,
      ...
      If[ display, Print[ "  ** Scheduled and allocated, one time dimension and one space dimension" ] ]
    ,
      (* One dimension of time and several dim of processors *)
      (lgIndexes > 2)&&(indexes[[1]] === "t")&&
    ...
    ,
      ...
      If[ display, Print[ "  ** Scheduled and allocated, one time dimension and two space dimension" ] ]
    ,
      (lgIndexes >= 2)&&(indexes[[1]] === "t")&&
    ...
    ,
      ...
      If[ display, Print[ "  ** Scheduled and not allocated, one time dimension" ] ]
    ,
    ...
    ,
      ...
      If[ display, Print[ "  ** Scheduled, more than one time dimension" ] ]
    ];

    (* Check various properties *)
    If[ isSpatialCaseEquQ[ lhs ], WriteString[ repFile, " spatial case "] ];

    If[ isConnexionEqQ[ eqPosition ], WriteString[repFile, " Connexion equation "] ];
    ...
    If[ mirrorEquation, 
      ...
      If[ display, Print[ "  ** Mirror equation" ] ]
    ];

    (* If the equation is local and identity and not mirror, 
       then it could be removed *)
    If[ idEquation && localEquation && !mirrorEquation, 
      WriteString[ repFile, "  ** Definition of "<>lhs<> " could be removed\n" ];
	If[ display, Prin[ "  ** Definition of "<>lhs<> " could be removed" ] ]
    ];
      ...
      ,
        Module[ { tv, m },
          ...
          , 
            ...
            If[ display, Print[ "  ** ",type," Register" ] ]
          ,
            allocated
          ,
            connectEq = True; 
            WriteString[ repFile, "  ** Connection\n" ];
            If[ display, Print[ "  ** Connection" ] ];
          ] (* Which *)
        ]
      ,
        (* *)
    ...
    If[ isControlEquQ[ eq ], 
      ...
      If[ display, Print[ "  ** Control equation" ] ];
    ...
    If[ isConnexionEqQ[ eq ], 
      ...
      If[ display, Print[ "  ** Connection equation" ] ]; 
    ...
    If[ isSpatialCaseEquQ[ eq ], 
      ...
      If[ display, Print[ "  ** Spatial case equation" ] ]; 
    ...
    If[ regEq||muxEq||operatorEq, 
     ...
     (* Modification. If the result is $Failed, then there is also an error *)
      Which[ 
        bounds ==={} || bounds === $Failed
      , 
      ...
      ,
        ...
        If[ dipslay, Print[ "  ** Valid from ", lb, " <= t <= ", ub ] ]
      ,
        bounds[[1,1]] =!= {}
      ,
        lb = bounds[[1,1]]; ub = bounds[[1,1]];
        WriteString[ repFile, "  ** Valid at t = ", lb, "\n" ];
        If[ display, Print[ "  ** Valid at t = ", lb ] ]
      ]
   ];
...
(*
 reportDep is a function called by report, it gives some information
 regarding the dependence in a equation. 
*)
Clear[ reportDep ];
reportDep[ repFile:_, dep:{___Integer}, opts:___Rule ]:=
  Module[ {mat, squareDep, uniform, uniformizable, display}, 
  ...
  If[ display, 
    Print[ "  ** Dependency: " ];
    Print[ ashow[ getPart[ $result, dep ], silent -> True ]];
    Print["\n     Occurrence: ", dep ]
  ];
  ...
  If[ squareDep, 
    ...
    If[ display, Print[ "    ** is square"] ]
  ,
    WriteString[ repFile, "    ** is not square\n" ];
    If[ display, Print[ "    ** is not square" ] ]
  ];
  ...
  If[ squareDep, 
    ...
    If[ mat1 =!= Null, uniformizable = True ]
  ];
  ...
  If[ uniformizable, 
    ...
    If[ display,
      Print[ "     ** can be uniformized"];
      Print[ "    "<>" -> Execute the command:" ];
      Print[ "    mkUniform[ "<>ToString[ dep ]<>" ]"]
    ];
...
Catch[
  ...
  *)
  ...
  Module[ {i, tempVar, TableVar, domns, nsp, defBefore, defAfter, sys2},
    i = 1; 
  ...
  (* If a pipeline vector was found, some cleaning is needed *)
  If[ success, 

    (* First, we simplify the new equation *)
    (* Get position of new equation *)
    If[ dbg, 
      asave[ sys1, "PC1.alpha" ];
      defBefore = getDefinition[ sys1, newVariableName ];
      Print["New equation before modifications: \n",
        ashow[ defBefore, silent -> True ] ]
    ];

    (* Get the position of the new equation *)
    posNewEq=
      First[ Position[sys1, equation[newVariableName,_]]];

    (* Find out new equation, simplify it, and replace *)
    newEq = getPart[ sys1, posNewEq ];
    newEq = simplifyInContext[ sys1, newEq ];
    sys1 = ReplacePart[ sys1, newEq, posNewEq ];

    If[ dbg, 
      asave[ sys1, "PC2.alpha" ];
      defAfter = getDefinition[ sys1, newVariableName ];
      Print["New equation after simplification: \n", 
        ashow[ defAfter, silent -> True ] ]; 
      If[ defAfter === defBefore, 
        Print["No modification was done" ],
        Print["Equation was modified..." ] 
      ];
    ];

    (* This is the new domain where the True part is
      initialized. We intersect is with each case branch below *)
    newDomInit = getPart[ sys1, Join[posNewEq,{2,1,1,1}]];
    If[ dbg, Print[ "newDomInit: \n", show[newDomInit,silent->True] ] ];

    (* Intersect domain of the second branch *)
    newCaseBranch = 
       getPart[ sys1, Join[ posNewEq, {2,1,1,2} ] ] /.
       (d1:_Alpha`domain :> DomIntersection[ d1, newDomInit ]); 

    sys2 = ReplacePart[sys1, newCaseBranch, Join[posNewEq,{2,1,1,2}] ];

    If[ dbg, 
      asave[ sys2, "PC3.alpha" ];

      If[ sys1 === sys2, 
        Print["No modification was done on sys1"],
        Print["System was modified and is in PC3.alpha"]
      ];
    ];
    sys1 = sys2;
    ...
    sys2 = normalizeDef[ sys1, variable ]; 

    If[ dbg, 
      asave[ sys2, "PC4.alpha" ];
      If[ sys1 === sys2, 
        Print["No modification was done on sys1"],
        Print["System was modified and is in PC4.alpha"]
      ];
    ];
    sys1 = sys2;

    (* We convexize the full system *)
    sys2 = convexizeAll[sys1]; 
    If[ dbg,
      If[ sys1 === sys2, 
        Print["No modification was done on sys1"],
        Print["System was modified and is in PC5.alpha"]
      ];
    ];
    sys1 = sys2;
    If[ dbg, asave[ sys1, "PC5.alpha" ] ];
    ...
    ,
      ...
      Check[ 
        sys2 = pipeControl[ sys1, newVariableName2, opts],
    ...
    ,
      ...
      (* else, realDimExp - nbParam = 0 we have only a temporal signal 
        In that case, we
      ...
      If[dbg, Print["before addlocal"]; ashow[ sys1 ]];
 		 
      sys2 = addlocal[ sys1, nameInit, Join[posNewEq,{2,1,1}]];

      If[dbg, Print["after addlocal"]; ashow[sys2]];
      ...
      *)
      ...
      dom = getDeclaration[sys1, nameInit][[3]];
      ...
      If[ dbg,
        If[ sys1 === sys2, 
          Print["No modification was done on sys1"],
          Print["System was modified and is in PC6.alpha"]
        ];
      ];
      sys1 = sys2;
      (* Save for debugging purposes *)
      If[ dbg, asave[ sys1, "PC6.alpha" ] ];
      ...
      *)
      ...
      If[ dbg,
        If[ sys1 === sys2, 
          Print["No modification was done on sys1"],
          Print["System was modified and is in PC7.alpha"]
        ];
      ];
      sys1 = sys2;
      (* Save for debugging purposes *)
      If[ dbg, asave[ sys1, "PC7.alpha" ] ];

      (* Finally we delocalize the control initialization
         variable (see below)     *)
      sys2 = delocalizeControl[ sys1, nameInit, opts];

      If[ dbg,
        If[ sys1 === sys2, 
          Print["No modification was done on sys1"],
          Print["System was modified and is in PC8.alpha"]
        ];
      ];
      sys1 = sys2;
      (* Save for debugging purposes *)
      If[ dbg, asave[ sys1, "PC8.alpha" ] ];
...
pipeControl[___]:=Message[pipeControl::params];
...
Catch[
  Module[ { dom, pos, newsys, dep, nameIn, paramlist, newInitDom, newDep,
    ...
    (* Check that variable exists in system *)
    If[ !MemberQ[ getLocalVars[ sys ], variable ],
      delocalize::unknownvar = "variable `1` is unknown";
      Throw[ Message[ delocalize::unknownvar, variable ]; sys ]
    ];
...
Catch[
  Module[{ verb, ctrlist, i, newsys, dbg, muteOpt, lvars },
    ...
    For[ i=1, i<=Length[ctrlist], i=i+1,
      If[ dbg, Print["---- Pipelining control for: ", ctrlist[[i]] ] ];
      Check[
