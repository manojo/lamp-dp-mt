...
t1=If [res===Table[0,{i,1,Length[res]}],
    Print["OK"];
    DeleteFile[Map[StringJoin["/tmp/",#] &,files]];
    DeleteFile[Map[StringJoin["/tmp/Good",#] &,files]];
    DeleteFile[Map[StringJoin["/tmp/T",#] &,files]];
    True,
    Print["Problem during comparison of vhdl files, see vhd files in /tmp/ "];
       DeleteFile[Map[StringJoin["/tmp/",#] &,files]];
       False];
...
Run["cd /tmp; cc /tmp/root065.c -o /tmp/root065 "];
Run["/tmp/root065 < data >/tmp/result"];
t2=testFunction[diffTailFile["result","/tmp/result"],0,"vhdl 2"];
SetDirectory["/tmp"];
DeleteFile[{"/tmp/root065.c","/tmp/root065.o","/tmp/root065","/tmp/result"}];
SetDirectory[dirGrob];
