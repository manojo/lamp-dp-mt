...
(* $Id: SubSystems.m,v 1.1 2004/06/16 15:14:08 risset Exp $
...
(* Standard head for CVS
	...
	$Source: /udd/alpha/CVS/alpha_beta/Mathematica/lib/Packages/Alpha/newPackages/SubSystems.m,v $
	$Revision: 1.1 $
	$Log: SubSystems.m,v $
...
SubSystems::usage = 
"Package. Contains fonctions for subsystem
...
"assignParameterValue[param_String,value_Integer,sys_Ast]\n Gives the
...
fixParameter::usage=
  "fixParameter[param_String,value_Integer,systName_String]
...
removeIdEqus::usage =
"removeIdEqus[] removes equations of the form A=B in $result introduced
...
subSystemUsedBy::usage = "subSystemUsedBy[sys_Alpha`system] returns the list
of names of subsystems used by system `sys' (default $result)"

topoSort::usage = "topoSort[lib:{__system}] returns the list of system
...
simplifyUseInputs::usage="simplifyUseInputs[sys:_Alpha`system] adds 
...
substDep[dep_Alpha`matrix, pAssgn_Alpha`matrix, pDimSub_Integer] :=
  If[ dep[[1]]==1, (* functions like (i,j->) : no parameter identity *)
      ...
      addParameterId[ composeAffines[ removeParameterId[ dep, pDimSub],
				      affExtHom[ pAssgn, 
						 Drop[ dep[[3]], -pDimSub ]
					       ] 
				    ],
...
substExpr[d_Alpha`matrix, 
	  ext_String, callerIds_List, extDomain_Alpha`domain, 
	  pAssgn_Alpha`matrix, pDimSub_Integer] := 
  substDep[ d, pAssgn, pDimSub]

substExpr[d_, ext_String, callerIds_List, extDomain_Alpha`domain, 
	  pAssgn_Alpha`matrix, pDimSub_Integer] := d
...
substEqus[ equs_List, ext_String, callerIds_List, extDomain_Alpha`domain, 
	   pAssgn_Alpha`matrix, pDimSub_Integer] :=
    equs   /.   {Alpha`equation[varname_String, exp_] :>
                    If [ MemberQ[callerIds, varname],
			 Alpha`equation[varname<>ext, 
					MapAll[ substExpr[ #, ext, 
					  callerIds, extDomain, 
					  pAssgn, pDimSub]&,
						exp]],
...
doInlineSS[ name_String, occurence_Integer, caller_Alpha`system, lib___,
		 rename_(*Boolean*), underscore_(*Boolean*),
                 verbose_(*Boolean*), 
		 countervalue_Integer] := 
Catch[
  Module[ {matches, ss, error, useNode, equsBeforeUse,equsAfterUse,
	  expString,subParams,callerParams,extDomain,paramAssign,newLoc,
	  callerIdList,subIdList,newEqus},
	 (* First, verify that the subsystem is there, that it is
	    there only once (otherwise issue a warning) *)
         If[ dbg, Print["underscore option is: ", underscore] ];
	 error = False;
	 ...
	 If[ error,
	     ...
	     (
	      ...
	      matches = Cases[ caller[[6]], (* the equations *)
			       Alpha`use[name,_,_,_,_] ];
	      If[ Length[matches]<occurence ,
		  (Message[ inlineSubsystem::NoSuchOcc, occurence, name];
		   Print["Aborting inlining."];
		   $Failed),
		  (
		   (* now we're sure that this occurence exists. *)
		   useNode = matches[[occurence]];
		   {equsBeforeUse, equsAfterUse} =
		   splitEquList[name, 
				{{},caller[[6]]},
				occurence];
		   subParams = ss[[2]];
		   callerParams = caller[[2]];
		   extDomain = useNode[[2]];
		   ...
		   If[ subParams[[1]] != paramAssign[[1]]-1,
			...
			Alpha`system[ caller[[1]], caller[[2]], caller[[3]],
				      caller[[4]], newLoc, newEqus ]
			) ]
		   ) ]
	      ) ]
       ]
...
inlineSubsystem[ name_String, options_List:{} ] := 
  Module[ {inlined},
	  inlined = doInlineSS[ name, 
				occurence/.options/.Options[inlineSubsystem],
				caller/.options/.Options[inlineSubsystem], 
				library/.options/.Options[inlineSubsystem], 
				rename/.options/.Options[inlineSubsystem],
				underscore/.options/.Options[inlineSubsystem],
				verbose/.options/.Options[inlineSubsystem],
				renameCounter/.options/.Options[inlineSubsystem]
			      ];
...
inlineAll[options_List:{}] := Module[
  {expanded, counter, ren, verb, lib, underS },
  ...
  While[ Cases[ expanded[[6]], Alpha`use[_,_,_,_,_] ] != {},
	 expanded = inlineSubsystem[
	   First[ Cases[ expanded[[6]], 
			 Alpha`use[_,_,_,_,_]   ]]  [[1]],
	   { caller :> expanded,
	     library -> lib, 
             underscore -> underS, 
	     rename -> ren, 
...
fixParameter[ident_String, value_Integer]:=
  Catch[
Module[{lib,prog,i},
	    ...
	    " has the same value everywhere and that $result contrains the top calling system of the library"];
           lib=Alpha`$library;
           Do[prog=lib[[i,1]];
          lib=Check[temp=fixParameter[ident, value,  prog , lib ],
...
fixParameter[ident_String, value_Integer,systName_String]:=
  Module[{},
    Alpha`$library=fixParameter[ident,value,systName,Alpha`$library];
...
fixParameter[ident_String, 
	     ...
	     lib_List] :=
Catch[
  Module[ {tmp,prog,parRank,lib1,newLib},
    ...
    If[ MatchQ[newLib,List[___Alpha`system]],
       Print[ident," suppressed in use of ",tmp[[1]],
	     " in library "]];
	 newLib,
	 lib1];
    lib1
