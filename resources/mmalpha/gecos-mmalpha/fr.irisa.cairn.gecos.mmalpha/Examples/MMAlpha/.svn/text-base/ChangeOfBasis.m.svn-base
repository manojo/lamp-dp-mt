...
*)
BeginPackage["Alpha`ChangeOfBasis`", {"Alpha`Domlib`","Alpha`",
                                      "Alpha`Options`",
				      "Alpha`Matrix`",
                                      "Alpha`Substitution`",
                                      "Alpha`Normalization`",
...
(* $Id: ChangeOfBasis.m,v 1.1 2004/06/16 15:14:04 risset Exp $
...
(* Standard head for CVS
	...
	$Author: risset $
	$Date: 2004/06/16 15:14:04 $
	$Source: /udd/alpha/CVS/alpha_beta/Mathematica/lib/Packages/Alpha/newPackages/ChangeOfBasis.m,v $
	$Revision: 1.1 $
	$Log: ChangeOfBasis.m,v $
	Revision 1.1  2004/06/16 15:14:04  risset
...
*)
ChangeOfBasis::usage =
"Package. Contains the definition of the \"change of basis\"
...
"changeOfBasis[var.fn_String] returns a copy of $result in which the
...
The  change of basis on the extension indices can be performed with the function extDomainCOB[] (see ?extDomainCOB)"
     ...
     extDomainCOB::usage="extDomainCOB[\"subSysName.(j->g(j,N))\"] applies the change of basis to all the variables 
...
res=Alpha`equation[LHSvar, exp_] :>
  Alpha`equation[LHSvar,
		 With[{LHSindices = Part[fn, 3]},
		      normalize[
			Alpha`affine[exp /. Alpha`var[LHSvar] -> 
				       Alpha`affine[Alpha`var[LHSvar], fn],
				     fnInverse]]
		      /. {Alpha`matrix[l_, c_, i_, m_]
			  -> Alpha`matrix[l, c, LHSindices, m],
			  Alpha`domain[d_, i_, p_]
...
useInputsRule[ LHSvar_String, fn_Alpha`matrix, fnInverse_Alpha`matrix ] :=
  Alpha`use[ id_, extDom_, paramAssgn_, inputs_, outputs_ ] :>
       Alpha`use[ id, extDom, paramAssgn, 
		 Map[ normalize,
		      inputs /. 
			Alpha`affine[Alpha`var[LHSvar],f1_] :>
			 Alpha`affine[Alpha`affine[Alpha`var[LHSvar], f1],fn]],
		 outputs ];
...
Catch[
  Module[{allvariables,oldDecl,optRecurse,optDebug,n1,decl},
  ...
  If [optDebug,Print["Entering traditional COB ***** "]];
  ...
  (* Matrix not unimodular *)
  If[ !unimodularQ[fn], Throw[Message[changeOfBasis::wrgmat]] ];
  If [optDebug,Print["pass unimodular test "]];
  ...
  If[n1-1=!=Length[decl[[2]]],Throw[Message[changeOfBasis::wrgdim]]
  ];
  If [optDebug,Print["pass dimension test ",LHSvar]];
  ...
  If [optDebug,Print["pass useOutputChek4GenCOB "]];
  res
 ]  (* Module *)
]; (* Catch *)
...
(** this function check wheter the variable to which is applied the
  change of basis is used as input or output to a subsystem. 
  If the options recurse is set, it calls useCOB, if not, if the
  variable is used as output, we have to add a locval variable 
  to enable the COB *) 
...
useOutputCheck4GenCOB[ 
  ...
  Module[{allVariables,optRecurse,optDebug,res, eqnUse},
	 ...
	 eqnUse=Select[ equas, Head[#]==Alpha`use & ];
	     If [optDebug,
	       Print["entering useOutputCheck4GenCOB  "];Print[sys]];
	 If [optRecurse,
	     If [optDebug,
...
Catch[
  ...
  matrices, we should have a DomZImage here *) 
    ...
    s1 = ReplacePart[sys, ReplacePart[ DomImage[d,m1], newidparam, 2 ], pos], (* change *)
...
(* Append the parameters to the list of new indices given by user *)
     ...
     If [Position[eqnUse[[2]],zpol,Infinity]=!={} && m1[[1]]=!=m1[[2]],
		Print["Warning, generalized change of basis with Zpols: please check the result"]];
     newUseDomain=DomImage[eqnUse[[2]],m1];	(* change *)
