...
(* Standard head for CVS
	...
	$Author: quinton $
	$Date: 2004/07/12 10:55:20 $
	$Source: /udd/alpha/CVS/alpha_beta/Mathematica/lib/Packages/Alpha/FarkasSchedule.m,v $
	$Revision: 1.26 $
...
readOneConstr::IncompatibleOptions= " There is incompatibility between
...
Module[{error,optDebug,optVerbose,optResolutionSoft,
	...
	paramVect,time1,dimNumber,tempFile1,temptempFile1},
...
multiSched[sys_Alpha`system, options___Rule]:=
  ...
  multiDimensional/.{options}/.Options[multiSched];
	 ...
	 If [schedFinal === Null,
	     multiSched1=Null,
	     If [optionList[[2]],
		 Print["Schedule Found has ", counter ," dimension"]];
	     multiSched1 = schedFinal[[1]];
	     Do [multiSched1 = 
		   MapThread[Append[#1,Part[#2,3]] &,
			     {multiSched1,schedFinal[[i]]}],
		 {i,2,counter}]];
	 reallyFinal = scheduleResult[2,multiSched1,{}];
	 If [(counter >= 1)&&(optionList[[2]]),
	     showSchedResult[reallyFinal]];
...
getRelativeNum[numInstr_Integer,numInstrVar_List,line0_List]:=
  Module[{rankOfVar,rankOfVarList,nbIn,nbOut,nbLoc,locOutIn,relativeNum},
	 ...
	 If [!(Length[rankOfVarList]>0),
	   ...
	   If [ nbIn < rankOfVar <= nbIn+nbOut,
		...
		If [nbIn+nbOut < rankOfVar <= nbIn+nbOut+nbLoc, 
		    locOutIn=1; 
		    relativeNum=rankOfVar-nbIn-nbOut ,
...
fichierPip[fich_String,mat_, refline_,options_List] :=
  Module[{matrice,fichInput1,fichInput2,fichOutput,result,
	  matrice2,silentOrNot,dir,command,timePip},
     ...
     writefile[fichInput1,matrice, refline,options];
	...
	Which[$OperatingSystem === "Unix",
          Run["sed -f $MMALPHA/sources/Sed/sup_antislash_pip.sed "<>fichInput1<>" > "<>fichInput2],
	...
	<>fichInput1<>" > "<>fichInput2;
	      ...
	      SetDirectory[dir]];
	If [ !options[[1]] ,
	     DeleteFile[fichInput1]];
	
	If [(!options[[2]]),
	    silentOrNot=" -s ",
	    silentOrNot="  "];
	
	 timePip=Timing[Which[$OperatingSystem === "Unix",
...
*)
	...
	(* If not debug mode, supress the temp files *)
...
(* This function get the result given by Pip for a LP 
   Problem generated for scheduling and write it in a more
   easy-to-read Manner, The strucyure of the file is known, the 
   refline (indicating how many phi,tau,alpha,llambda,... are 
   present in the file) is given as second argument 
   and. We should retreive the value of the last variable to all taus
   ans alphas *)

Clear[readPipResult]

readPipResult[file_String,
	      refline_List
		(* {nbphi_List,nbTau_List,nbAlpha_List,rest___} *) ,
	      options_List]:=
  Module[{res,nbPhi,nbInstr,nbTotTau,listNegCoef,
	  stream1,offset,offsetAlpha,res2,resBis,res1,posList,posTheList},
	 ...
	 (* res is a list containing the record encountered 
	    the  record which indicates the beguinning of the coefs 
	    is {"List"}, the other are alternatively 
	    {"\n"} and {"Nb1","Nb2"} where Nb1 is the coef of the Big Par
	    and Nb2 is the coeff of the vaiable 
	    *)
	 ...
	 If [Apply[Or,
	   ...
	   (* offset is the offset after what the coeff of the tau starts,
	    it coreesponds to the coeffs of the objective function but
	 ...
	 listNegCoef = Select[Map[ToExpression ,resBis],
			      Negative];
	 ...
	 If [options[[2]],Print["Shift coef: ",shiftCoef]];
...
(* Build the schedule from the stream, recursive, take as paramemter 
   ...
   return the scheduke found in the form 
   <schedFoundList>
...
(*  
 for affine by variable schedule (schedukleType 1):      
