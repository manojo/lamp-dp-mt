...
(* $Id: Normalization.m,v 1.34 2004/06/16 14:57:29 risset Exp $ Z. Chamski & M. Le Bouder
...
(* Standard head for CVS
	...
	$Author: risset $
	$Date: 2004/06/16 14:57:29 $
	$Source: /udd/alpha/CVS/alpha_beta/Mathematica/lib/Packages/Alpha/Normalization.m,v $
	$Revision: 1.34 $
...
"Package containing the definitions of normalization rules along with basic
normalizing functions: normalize, normalize0, normalizeDef, normalizeDef0,
checkRestrictions, normalizeInCtxt, normalizeInCtxt0, minRestrictInCtxt,
checkRestrictionsInCtxt, normalizeQ, normalize0Q, normalizeA, 
FusionneCase, FusionneVars, simplifyInContext"

Alpha`Normalization::qv =
  "functions: normalize, normalizeDef, checkRestrictions,
  normalSubstit; normalize0, normalizeDef0, checkRestrictions0;
\nvariables: normalizationRules, normalizationRules0;
\npackages: \"Alpha`Substitution`\", \"Alpha`Tables`\", \"Alpha`Matrix`\"."
...
normalize::qv =
"functions: normalizeDef;\nvariables: normalizationRules, $result;
\npackages: \"Alpha`Normalization`\", \"Alpha`Matrix`\".";
...
normalize0::qv =
"functions: normalizeDef;\nvariables: normalizationRules0, $result;
\npackages: \"Alpha`Normalization`\", \"Alpha`Matrix`\"."

normalizeDef::usage = 
"normalizeDef[symbol] normalizes the definition
of ALPHA variable symbol in
...
normalizeDef0::usage =
"normalizeDef0[symbol] normalizes the definition
  of ALPHA variable `symbol' in
...
ALPHA expression towards the CRD (Case-Restriction-Dependence) normal form."

normalizationRules::note =
  "'normalizationRules' can be extended by appending/prepending new rules to
the initial list. The rules are named and have the form 'name = exp1 :> exp2'.
"

normalizationRules::qv =
  "functions: normalize; \nvariables: none; \npackages:
 \"Alpha`Normalization`\"."
...
normalizationRules0::qv =
  "functions: normalize0; \nvariables: none; \npackages:
 \"Alpha`Normalization`\"."

(* simplifyRestrictions::usage =
"simplifyRestructions[ast,variable] simplifies all the restrictions in an ALPHA program or expression
wrt. the declaration domain of the corresponding LHS variable.
Defaults ast is 'Alpha`$result'. variable is a LHS variable name 
(OPTIONAL, symbol or string.)
REMARK: the LHS variable name forces the declaration domain to be that of
the variable given. This form should always be used in conjunction with an
ALPHA program."
...
normalizeInCtxt::qv =
  "functions: minRestrictInCtxt, checkRestrictionsInCtxt, \n  normalize;
\nvariables: none; \npackages: \"Alpha`Normalization`\"."
...
program in the context of a specific domain. Returns the simplified expression."
normalizeQ::usage =
...
minRestrictInCtxt::qv =
  "functions: DomSimplify; \nvariables: none; \npackages:
 \"Global`\"."
...
Alpha`Normalizationbugs = "Not know...";
...
(* Restriction: 
   - le domaine de la restriction devient le domaine simplifié
   dans le contexte fourni.
   - on appelle la fonction sur l'expression qui suit la 
   restriction, en effectuant une intersection entre le 
   contexte et le domaine.
 *)
...
Module[ { dbg, d1, d2 },
  ...
  If[ dbg, Print[ "Nouveau domaine : ", ashow[ d1, silent->True ] ] ];
  d2 = DomIntersection[ dom, context ];
  If[ dbg, Print[ "Nouveau contexte : ", ashow[ d2, silent->True ] ] ];
...
(* I add a function for cleanning up the result of DomMatrixSimplify
   this function scans the parameter part of the affine function ans
   checks that they are correct (no other indices are involved) if
   they are not correct, it check the constext and replace it *)
...
Catch[
  Module[{ formatOpt, res, dbg, simpl1, simpl2, simpl3},
	 ...
	 normalize[convexizeAll[simplifyInContext[sys]]]],sys],
      ...
      res = 
        Check[correctAffineFunctions[simpl3],
          ...
          Message[ simplifySystem::simpl3 ]; Throw[ simpl3 ]
        ],
