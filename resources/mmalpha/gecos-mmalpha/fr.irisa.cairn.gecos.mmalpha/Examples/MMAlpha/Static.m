...
(* Static analysis of an ALPHA program. The commands are: 
   ...
   temporarily).
*)
(* $Id: Static.m,v 1.1 2004/06/16 15:14:08 risset Exp $
...
(* Standard head for CVS
	...
	$Source: /udd/alpha/CVS/alpha_beta/Mathematica/lib/Packages/Alpha/newPackages/Static.m,v $
	$Revision: 1.1 $
	$Log: Static.m,v $
...
Static::usage = "The Static package contains functions for the 
...
analyze::usage =
"analyze[] performs a static analysis of $results, and returns True if
...
"dep[] generates a dependency table for $result.  dep[sys] generates a
...
dep1::usage =
"dep1[sys_, {occur:{_Integer..}, occurs___}] generates dependency
...
Module[
  ...
  Catch[
    ...
    (*    If[ verb, Print[ casePositions ] ]; *)
    equationPosition = First[ Position[ sys, e ] ]; 
    (*     If[ verb, Print[ equationPosition ] ];*)
    ...
    (* subdom is the difference between the domain of the lhs 
       and the rhs *)
    subdom = DomDifference[ addParameterDomain[ lhsdom, paramdom ],
				   rhsdom];
    paramlist = If[Head[paramdom[[3,1]]]===Alpha`pol,paramdom[[2]],paramdom[[3,1,1,3]]];	(* change *)
    ...
    (* If subdom is not empty, this means that there 
...
checkEquation[___] := Message[ checkEquation::params ];
...
dep[ sys: Alpha`system[ name_, param_, in_, out_, loc_, eqns_ ],
  optionsList_List]:=
Catch[
  Module[{ depRes1, optSimplify, uses, optsubsystems, occurs, dbg , dp1 },
    ...
    With[ {dp1 = dep1[ sys, uses, optionsList]},
      If[ optsubsystems && dp1 =!= {}, 
          dtable[ depRes1, Select[dep1[ sys, uses, optionsList ],#=!={} & ]], 
          Alpha`dtable[ depRes1 ] ] ]
    ]
];

dep[a___]:= (Message[dep::WrongArg,a];Throw["ERROR"])
...
dep1[ sys_, occ: {_Integer..}, opts:{___Rule}] := 
Module[ { rhsvar, rhsdom, lhsvar, context, intersect, preim, pc ,simplifyEq},
