...
trait ADPParsers extends BaseParsers { this:Signature =>
  ...
  implicit def parserADP[T](p1:Parser[T])(implicit mT:Manifest[T]) = new ParserADP(p1)
  class ParserADP[T](p1:Parser[T],idx:(Int,Int,Int,Int)=null)(implicit mT:Manifest[T]) {
    def ~ [U](p2:Parser[U])(implicit mU:Manifest[U]) = if (idx!=null) new Concat(p1,p2,0) { override lazy val indices=idx } else new Concat(p1,p2,0)
  ...
  def el = new Terminal[Alphabet](1,1,(i:Var,j:Var) => (Nil,"in["+i+"]"))(mAlphabet) {
