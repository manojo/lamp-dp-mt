...
trait Signature  {
  ...
  implicit val mAlphabet:Manifest[Alphabet]
  implicit val mAnswer:Manifest[Answer]
...
trait BaseParsers extends ScalaOpsPkg with TupledFunctions with UncheckedOps with LiftPrimitives with LiftString with LiftVariables { this:Signature =>
  ...
  sealed abstract class Parser[T](implicit mT:Manifest[T]) extends (Subword => List[(T,Backtrack)]) {
    ...
    final def ^^[U](f: T => U)(implicit mU:Manifest[U]) = new Map(this,f)(mT,mU)
  ...
  class Tabulate(in: => Parser[Answer], val name:String) extends Parser[Answer]()(mAnswer) {
    lazy val inner = in
    val (alt,cat) = (1,0)
  ...
  abstract case class Terminal[T](min:Int,max:Int, f:(Var,Var)=>(List[Cond],String))(implicit mT:Manifest[T]) extends Parser[T] {
  ...
  case class Aggregate[T](inner:Parser[T], h: List[T] => List[T])(implicit mT:Manifest[T]) extends Parser[T] {
    def min = inner.min
    def max = inner.max
  ...
  case class Filter[T](inner:Parser[T], pred: Subword => Boolean)(implicit mT:Manifest[T]) extends Parser[T] {
    def min = inner.min
    def max = inner.max
    lazy val (alt,cat) = (inner.alt,inner.cat)
  ...
  case class Map[T,U](inner:Parser[T], f: T => U)(implicit mT:Manifest[T], mU:Manifest[U]) extends Parser[U] {
    def min = inner.min
    def max = inner.max
    lazy val (alt,cat) = (inner.alt,inner.cat)
  ...
  case class Or[T](left:Parser[T], right:Parser[T])(implicit mT:Manifest[T]) extends Parser[T] {
  ...
  case class Concat[T,U](left: Parser[T], right:Parser[U], track:Int)(implicit mT:Manifest[T], mU:Manifest[U]) extends Parser[(T,U)] {
