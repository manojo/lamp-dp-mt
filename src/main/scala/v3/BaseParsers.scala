...
trait BaseParsers extends CodeGen { this:Signature =>
  ...
  val axiom:Tabulate // initial parser to be applied (must be a tabulation)
  ...
  def p_or[T](inner:Parser[T], that: Parser[T]) = new Parser[T] {
    ...
    override def unapply(sw:Subword,bt:Backtrack) = bt match {
      case (r,idx) => var a = inner.tree.alt-1; if (r<=a) inner.unapply(sw,(r,idx)) else that.unapply(sw,(r-a,idx)) 
    }
  ...
  def p_filter[T](inner:Parser[T], p: Subword => Boolean) = new Parser[T] {
    ...
    def apply(sw: Subword) = if(p(sw)) inner(sw) else Nil
  ...
  def p_concat[T,U](inner:Parser[T], that: Parser[U], indices:(Int,Int,Int,Int)) = new Parser[(T,U)] {
    ...
    override def unapply(sw:Subword,bt:Backtrack) = {
      ...
      for (
        (x,xb) <- inner.unapply(swl,bl);
        (y,yb) <- that.unapply(swr,br)
      ) yield (((x,y), xb:::yb))
    }
  ...
  abstract class Parser[T] extends (Subword => List[(T,Backtrack)]) with Treeable { inner =>
    ...
    def unapply(sw:Subword,bt:Backtrack): List[(T, List[BTItem])] = apply(sw).map{case(t,b)=>(t,Nil)} // default for terminals
