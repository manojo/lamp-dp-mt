    ...
    withStream(out) {
      for ((name, tuple) <- encounteredTuples) {
        stream.println{ tuple match {
          case e2@ETuple2(a,b) =>
            "typedef struct {"+ remap(e2.m1) + " t1; " + remap(e2.m2) + " t2; } "+name+";"
          case e3@ETuple3(a,b,c) =>
            "typedef struct {"+ remap(e3.m1) + " t1; " + remap(e3.m2) + " t2; "+ remap(e3.m3) +" t3; } "+name+";"
          case e4@ETuple4(a,b,c,d) =>
            "typedef struct {"+ remap(e4.m1) + " t1; " + remap(e4.m2) + " t2; "+ remap(e4.m3) +" t3; " + remap(e4.m4) +" t4; } "+name+";"
          case e5@ETuple5(a,b,c,d,e) =>
            "typedef struct {"+ remap(e5.m1) + " t1; " + remap(e5.m2) + " t2; "+ remap(e5.m3) +" t3; " + remap(e5.m4) +" t4; " + remap(e5.m5) +" t5 } "+name+";"
        }}
...
trait CGenTupleOps extends CGenBase with CLikeGenTupleOps