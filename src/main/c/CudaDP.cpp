 ...
 */
JNIEXPORT void JNICALL Java_CudaDP_inArrays
 ...
 */
JNIEXPORT void JNICALL Java_CudaDP_inStrings
 ...
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_CudaDP_computeMatrix
  (JNIEnv *, jobject);
 ...
 * Signature: ()J
 */
JNIEXPORT jlong JNICALL Java_CudaDP_getScore
  (JNIEnv *, jobject);
 ...
 */
JNIEXPORT jobjectArray JNICALL Java_CudaDP_getBacktrack
  (JNIEnv *, jobject);
 ...
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_CudaDP_free
  (JNIEnv *, jobject);
...
static int inArray(JNIEnv* env, jobjectArray input, TI** in) {
	...
	*in=(TI*)malloc(size*sizeof(TI)); assert(*in);
	...
	/*
    jmethodID mid = env->GetMethodID(cls, "productArity", "()I");
    jint arity = env->CallObjectMethod(elem, mid);
    printf("arity = %d\n",arity);
    */

    // GEN: generate the following
    jmethodID ms[] = {
    	env->GetMethodID(cls, "_1", "()I"),
    	env->GetMethodID(cls, "_2", "()I"),
    };

	for (i=0;i<size;++i) {
		elem = env->GetObjectArrayElement(input, i);
		TI* e = &((*in)[i]);
		e->_1 = (long) env->CallObjectMethod(elem, ms[0]);
		e->_2 = (long) env->CallObjectMethod(elem, ms[1]);
	}
